...
BRUSH_SPACING = 24.0          # 24 is a good start
BRUSH_SCALE = 80.0            # keep >= spacing so stamps overlap

# smoothing applied to height field (in grid-cell units)
SIGMA_LAND = 1.4              # overall smoothing
SIGMA_WATER = 6.0             # extra smoothing for "missing" areas (likely water)
...
RELIEF_MULT = 1.0             # keep 1.0 now that values are meters
...
while x_pos <= 1000.0:
    ...
    while z_pos <= 1000.0:
        ...
        if height_val_m > STAMP_EPS:
            landscape_entries.append(
                {
                    "tool": 1,  # IMPORTANT: matches your RAISE/LOWER test files
                    ...
                    "value": float(height_val_m),  # METERS
                    "holeId": -1,
                }
            )
...
# Turn off procedural hills/noise so only your LiDAR stamps shape the plot
...
course.set_name("TEST - LAZ LANDSCAPING (METERS)")
...
print("Done. Load 'TEST - LAZ LANDSCAPING (METERS)' in 2K25.")
